...
void WriteHstHeader()
  {
   ...
   int    hst_period=Period();
   int    hst_digits=Digits;
   int    i_unused[16];
   //----  
   ExtHstHandle=FileOpen(c_symbol+hst_period+".hst", FILE_BIN|FILE_WRITE);
   if(ExtHstHandle < 0) return;
   //---- write history file header
   c_copyright="(C)opyright 2003, MetaQuotes Software Corp.";
   FileWriteInteger(ExtHstHandle, hst_version, LONG_VALUE);
   FileWriteString(ExtHstHandle, c_copyright, 64);
   FileWriteString(ExtHstHandle, c_symbol, 12);
   FileWriteInteger(ExtHstHandle, hst_period, LONG_VALUE);
   FileWriteInteger(ExtHstHandle, hst_digits, LONG_VALUE);
