...
//| Grid EA with dynamic range, MTF signals, visual preview, and     |
//| expanded risk/regime controls.                                   |
//|                                                                  |
//| New since last version:                                          |
//|  - Risk preset selector (RiskAware/Natural/Aggressive)           |
//|  - Per-order Money SL (last safeguard)                           |
//|  - ATR+ADX adaptive SL (optional)                                |
//|  - Grid-aware SL (optional; rung-based)                          |
//|  - Squeeze gating with hysteresis and Pause zone                 |
//|  - Edge-guard (skip orders too near RT/RB)                       |
//|  - ATR-scaled breakout buffer + close-confirm bars               |
//|  - Auto-rebuild on regime change / recenter (delete&reseed)      |
//|  - Per-order %TP + ATR trailing (optional)                       |
//|  - Staged PlaceAllGridLevels (only when range-like & stable)     |
//|  - Enhanced trade markers retained                               |
//+------------------------------------------------------------------+
#property strict

/******************** USER INPUTS **********************************/
input bool AutoProceedInTester = true;  // auto skip preview/pause in Strategy Tester & Optimization
...
input bool     AllowHedge             = false;      // place both sides if true
input bool     AllowNewTrades         = true;

// Preset (templates for effective values; underlying inputs still used if UseRiskPreset=false)
...
// ZigZag parameters (only when EnvelopeMode = Envelope_ZigZagPivots)
...
input int             ST_StableBars   = 6;   // bars without direction change to be "stable"
...
input double DollarsPerStep           = 5.0;    // used when Step_FixedDollars
input int    FixedStepPoints          = 150;    // used when Step_FixedPoints
input double StepSmoothingFactor      = 0.30;   // 0..1 EMA smoothing on step; set 0 to disable
input bool   RespectLevelsEachSide    = true;   // cap step so L rungs fit inside tighter half

/*** Grid ***/
enum eGridMode { Grid_Reversion_Limit=0, Grid_Breakout_Stop=1 }; // base mode (may be overridden)
input eGridMode GridMode              = Grid_Reversion_Limit;
input int       GridLevelsEachSide    = 6;

// Breakout buffer (choose fixed points or ATR-scaled)
input int       EntryBufferPoints     = 5;       // base buffer in broker points
input bool      UseATRScaledBuffer    = false;
input ENUM_TIMEFRAMES BufferATR_TF    = PERIOD_H1;
input int       BufferATR_Period      = 14;
input double    BufferATR_Mult        = 0.25;    // trigger distance = max(EntryBufferPoints, ATR*mult in points)
input int       BreakoutCloseConfirmBars = 0;    // 0 = no confirm; N bars must close beyond rung to arm stop

// PlaceAllGridLevels staging
enum eLevelsMode { Levels_Off=0, Levels_NearestOnly=1, Levels_All=2, Levels_RangeOnly=3 };
input eLevelsMode PlaceLevelsMode     = Levels_NearestOnly; // RangeOnly will place all only when PSI>PSI_High AND ST stable

/*** Edge guard ***/
input double EdgeGuardPercentToRange  = 10.0;  // skip orders if target/TP is within X% of RT/RB
...
/*** Per-order risk (SL) ***/
input bool   UsePerOrderSLMoney       = false;
input double PerOrderSLMoney          = 25.0;   // last safeguard per ticket

input bool   UseATR_SL                = false;
...
input double ADX_ScaleAlpha           = 0.4;    // scale = 1 + alpha * clamp((ADX-ADX0)/Range,0,1)

input bool   UseGridAwareSL           = false;
input int    GridAwareRungsBeyond     = 1;      // rungs beyond the immediate adverse rung
input int    GridAwareBufferPoints    = 20;

/*** Per-order exits ***/
input bool   UsePerOrderTP            = true;   // fixed points TP (legacy)
input int    PerOrderTP_InPoints      = 300;
input bool   TP_ToNextGridLevel       = true;   // rung-based TP
input bool   UsePerOrderTP_Percent    = false;  // percent of entry price (e.g., 0.2% = 0.2)
input double PerOrderTP_Percent       = 0.20;

input bool   UseATR_Trailing          = false;  // ATR trailing for open positions
input ENUM_TIMEFRAMES ATRTrail_TF     = PERIOD_H1;
input int    ATRTrail_Period          = 14;
input double ATRTrail_Mult            = 1.5;
input int    TrailStepMinPoints       = 20;     // do not adjust SL unless improves at least this many points
...
double   g_cachedPsi       = EMPTY_VALUE; // last closed PSI cached for panel
...
void   TryBasketBreakeven_Global(double needMoney, double needPct);
void   TryBasketBreakeven_SameSide(double needMoney, double needPct);   // stub -> Global
void   TryBasketBreakeven_OppositePair(double needMoney, double needPct); // stub -> Global
...
int    EffectiveModeByStrategy(double psiNow); // 0=Reversion, 1=Breakout, -1=Pause
void   RedrawVisuals();
...
int    BufferPointsEffective();                   // buffer by ATR or fixed
...
double ComputeSLPrice(int side,double entryPrice); // tightest enabled SL
...
double GetATR(ENUM_TIMEFRAMES tf,int period,int shift)
{
   // iATR returns ATR on shift (0 current, 1 last closed)
...
int StableTrendBars(int curDir)
{
   // crude stability using chart bars since last flip
...
double ComputeSLPrice(int side,double entryPrice)
{
   ...
   // Grid-aware SL: one rung beyond adverse side
   if(UseGridAwareSL && step>0)
   {
      int rungs = MathMax(1, GridAwareRungsBeyond);
      double adverse = (side>0)? (g_rangeCtr - step*Point) : (g_rangeCtr + step*Point); // immediate adverse rung approximated by center±step
      // project SL rungs beyond from entry, not center: move rungs in adverse direction
      double pts = rungs * step;
      if(pts>0 && pts<bestDistPts) bestDistPts=pts;
   }

   if(bestDistPts>=1e12) return 0.0; // no SL
   double sl = (side>0)? (entryPrice - bestDistPts*Point) : (entryPrice + bestDistPts*Point);
   return NormalizeDouble(sl, DigitsAdjust);
}
double ComputeTPPrice(int side,double entryPrice,int stepPts)
{
   // Priority: %TP > fixed points > rung TP (if rung TP disabled)
...
// --- Helpers to safely modify SL/TP with retries and broker constraints ---
bool NormalizeStopsForBroker(int type, double &sl, double &tp, int digitsAdj)
{
   ...
   double freeze    = freezePts * Point;

   RefreshRates();
   double curBid = Bid, curAsk = Ask;

   // Adjust to satisfy minimum distances; keep zeros as "no change"
   if(type==OP_BUY)
   {
...
bool ModifyOrderSLTPWithRetry(int ticket, double newSL, double newTP, color col, int digitsAdj)
{
   ...
   for(int attempt=0; attempt<5; attempt++)
   {
      ...
      if(err==ERR_OFF_QUOTES      || // 136
         err==ERR_SERVER_BUSY     || // 4
         err==ERR_TRADE_CONTEXT_BUSY || // 146
         err==ERR_PRICE_CHANGED)     // 135
      { Sleep(250); RefreshRates(); continue; }

      // Invalid stops/price: try once more after normalization in next loop
...
bool PlacePending(int side,int type,double price,string reason,int trendDir)
{
   ...
   // Compute SL/TP
   double sl = ComputeSLPrice(side, price);
   double tp = ComputeTPPrice(side, price, step);
...
void ManageEntries(int effMode,int trendDir,double &buyLvls[],int nB,double &sellLvls[],int nS)
{
   if(effMode==-1) return; // Pause band
   ...
   bool placeAll=false;
   switch(PlaceLevelsMode)
   {
      case Levels_All: placeAll=true; break;
      case Levels_NearestOnly: placeAll=false; break;
      case Levels_Off: return;
      case Levels_RangeOnly:
      {
         int stPrev=0; int curDir = UseTrendFilter ? SupertrendDirectionTF(TrendTF, ST_ATR_Period, ST_Factor, 400, stPrev) : 0;
         int stableBars = StableTrendBars(curDir);
         placeAll = (IsNum(g_cachedPsi) && g_cachedPsi>PSI_High && stableBars>=ST_StableBars);
      } break;
   }

   // Effective buffer (for breakout)
   int effBufferPts = BufferPointsEffective();
   ...
   if(AllowSide(+1,trendDir) && buyOpen+buyPend < MaxOrdersPerSide)
   {
      for(int i=0;i<nB;i++)
      {
         double baseLvl=buyLvls[i];
         int type; bool eligible=false; double lvl=baseLvl;

         if(effMode==0) // Pullback
         {
            type=OP_BUYLIMIT; eligible=(baseLvl < bid - Point);
         }
         else // Breakout
         {
            if(!ConfirmCloseBeyondRung(+1, baseLvl, BreakoutCloseConfirmBars)) continue;
            type=OP_BUYSTOP;
            double trig=baseLvl + effBufferPts*Point;
            eligible=(trig > ask + Point);
            lvl=trig;
         }
   ...
   if(AllowSide(-1,trendDir) && sellOpen+sellPend < MaxOrdersPerSide)
   {
      for(int j=0;j<nS;j++)
      {
         double baseLvl=sellLvls[j];
         int type; bool eligible=false; double lvl=baseLvl;

         if(effMode==0) // Pullback
         {
            type=OP_SELLLIMIT; eligible=(baseLvl > ask + Point);
         }
         else // Breakout
         {
            if(!ConfirmCloseBeyondRung(-1, baseLvl, BreakoutCloseConfirmBars)) continue;
...
void TryBasketBreakeven_Global(double needMoney, double needPct)
{
   ...
   bool okMoney=(needMoney>0 && net>=needMoney);
   bool okPct=(needPct>0 && eq>0 && (100.0*net/eq)>=needPct);
   if(!(okMoney||okPct)) return;
   CloseAllPositions();
}
void TryBasketBreakeven_SameSide(double needMoney, double needPct)
{
   // For now, fallback to global
   TryBasketBreakeven_Global(needMoney, needPct);
}
void TryBasketBreakeven_OppositePair(double needMoney, double needPct)
{
   // For now, fallback to global
   TryBasketBreakeven_Global(needMoney, needPct);
}
void TryBasketBreakeven()
{
   ...
   {
     case Basket_Global:           TryBasketBreakeven_Global(BasketBE_ProfitMoney, BasketBE_ProfitPercent); break;
     case Basket_SameSideGrouping: TryBasketBreakeven_SameSide(BasketBE_ProfitMoney, BasketBE_ProfitPercent); break;
     case Basket_OppositePairing:  TryBasketBreakeven_OppositePair(BasketBE_ProfitMoney, BasketBE_ProfitPercent); break;
   }
}
...
void ShowPreviewPanelNow(string reason, int effMode, int trendDir, double psiNow, double stepPrice, int stepPts)
{
   ...
   string t3 = "Squeeze PSI (last closed): "+(IsNum(psiNow)? DoubleToString(psiNow,2):"n/a")+
               "   Band: ["+DoubleToString(PSI_Low,1)+", "+DoubleToString(PSI_High,1)+"]";
...
void ApplyRiskPresetIfEnabled()
{
   ...
   // We do not overwrite user Inputs; instead, we rely on a few effective behaviors
   // driven by the selected preset. Minimal, conservative changes:
   if(RiskPreset==Preset_RiskAware)
   {
      // Make breakout cleaner and avoid edges
      // (These are used by logic directly, we read inputs as-is.)
      // Suggest: keep UseATRScaledBuffer=true; BreakoutCloseConfirmBars=1; EdgeGuard 12%+
      // Because we can't change input vars, we only warn via journal if suboptimal.
   ...
   else if(RiskPreset==Preset_Aggressive)
   {
      // Allow more placements; user may reduce guards during optimization
      // No hard overrides; keep logic safe.
   }
   // Natural: leave as provided.
}
...
int OnInit()
{
   ...
   if(inTester && AutoProceedInTester)
   ...
   // Only show preview when not running tester OR when user wants Visual mode to click
...
void OnTick()
{
   ...
   // ATR trailing for open positions (optional)
   if(UseATR_Trailing)
   {
